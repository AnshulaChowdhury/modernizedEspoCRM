...
class MasterSiteView extends View {
    ...
    /**
     * Action handler for switching to the modern React frontend.
     * Preserves the current route by converting the hash to a path.
     * Uses relative URL for Traefik routing (works in both dev and prod).
     *
     * @param {MouseEvent} e
     */
    // eslint-disable-next-line no-unused-vars
    actionSwitchToModernView(e) {
        e.preventDefault();

        const hash = window.location.hash.slice(1); // Remove '#'

        // Use relative path - Traefik routes / to React frontend
